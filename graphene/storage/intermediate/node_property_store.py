class NodePropertyStore:
    ...
    def __getitem__(self, key):
        cur_node = self.node_store.node_at_index(key)
        if cur_node is None:
            return None
        properties = []
        cur_prop = self.prop_store.property_at_index(cur_node.propId)
        while cur_prop is not None:
            properties.append(cur_prop)
            cur_prop = self.prop_store.property_at_index(cur_prop.nextPropId)
